...
const variantShells = {
  'listing-directory': 'bg-neutral-50 text-neutral-900',
  'listing-showcase': 'bg-neutral-50 text-neutral-900',
  'article-editorial': 'bg-neutral-50 text-neutral-900',
  'article-journal': 'bg-neutral-50 text-neutral-900',
  'image-masonry': 'bg-[linear-gradient(180deg,#09101d_0%,#111c2f_100%)] text-white',
  'image-portfolio': 'bg-[linear-gradient(180deg,#07111f_0%,#13203a_100%)] text-white',
  'profile-creator': 'bg-[linear-gradient(180deg,#0a1120_0%,#101c34_100%)] text-white',
  'profile-business': 'bg-neutral-50 text-neutral-900',
  'classified-bulletin': 'bg-neutral-50 text-neutral-900',
  'classified-market': 'bg-neutral-50 text-neutral-900',
  'sbm-curation': 'bg-neutral-50 text-neutral-900',
  'sbm-library': 'bg-neutral-50 text-neutral-900',
} as const
...
export async function TaskListPage({ task, category }: { task: TaskKey; category?: string }) {
  ...
  const shellClass = variantShells[layoutKey as keyof typeof variantShells] || 'bg-neutral-50 text-neutral-900'
  ...
  const lightUi = {
    ...
    panel: 'rounded-[2rem] border border-neutral-200 bg-white shadow-sm',
    soft: 'rounded-[2rem] border border-neutral-200 bg-neutral-50',
    input: 'rounded-xl border border-neutral-200 bg-white text-neutral-900',
    button: 'rounded-xl bg-[#ff8c00] font-semibold text-white hover:bg-[#e67e00]',
  }
      ...
      <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {layoutKey === 'listing-directory' || layoutKey === 'listing-showcase' ? (
          <section className="mb-12 grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
            <div className={`rounded-[2rem] p-7 shadow-[0_24px_70px_rgba(15,23,42,0.07)] ${ui.panel}`}>
              ...
              <h1 className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-neutral-900">{taskConfig?.description || 'Latest posts'}</h1>
        ...
        {layoutKey === 'article-editorial' || layoutKey === 'article-journal' ? (
          <section className="mb-12 grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
            <div>
              ...
              <h1 className="mt-3 max-w-4xl text-5xl font-semibold tracking-[-0.05em] text-neutral-900">{taskConfig?.description || 'Latest posts'}</h1>
        ...
        {layoutKey === 'image-masonry' || layoutKey === 'image-portfolio' ? (
          <section className="mb-12 grid gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
            <div>
              ...
              <h1 className="mt-5 text-5xl font-semibold tracking-[-0.05em] text-white">{taskConfig?.description || 'Latest posts'}</h1>
              ...
              <div>
                ...
                <h1
                  className={cn(
                    ...
                    layoutKey === 'profile-creator' ? 'text-white' : 'text-neutral-900',
                  )}
                >
        ...
        {layoutKey === 'classified-bulletin' || layoutKey === 'classified-market' ? (
          <section className="mb-12 grid gap-4 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
            <div className={`rounded-[1.8rem] p-6 ${ui.panel}`}>
              <p className={`text-xs uppercase tracking-[0.3em] ${ui.muted}`}>{taskConfig?.label || task}</p>
              <h1 className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-neutral-900">Fast-moving notices, offers, and responses in a compact board format.</h1>
            </div>
        ...
        {layoutKey === 'sbm-curation' || layoutKey === 'sbm-library' ? (
          <section className="mb-12 grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
            <div>
              <p className={`text-xs uppercase tracking-[0.3em] ${ui.muted}`}>{taskConfig?.label || task}</p>
              <h1 className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-neutral-900">Curated resources arranged more like collections than a generic post feed.</h1>
        ...
        {intro ? (
          <section className={`mb-12 rounded-[2rem] p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)] sm:p-8 ${ui.panel}`}>
            <h2 className="text-2xl font-semibold text-neutral-900">{intro.title}</h2>
            {intro.paragraphs.map((paragraph) => (
              <p key={paragraph.slice(0, 40)} className={`mt-4 text-sm leading-7 ${ui.muted}`}>{paragraph}</p>
            ))}
            <div className="mt-4 flex flex-wrap gap-4 text-sm">
              {intro.links.map((link) => (
                <a key={link.href} href={link.href} className="font-semibold text-[#ff8c00] hover:underline">
                  {link.label}
                </a>
              ))}
            </div>
